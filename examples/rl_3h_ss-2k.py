#max to true max
#moves_dmg_multiplier updated for stab
...
class SimpleRLPlayer(Gen7EnvSinglePlayer):
    def embed_battle(self, battle):
        ...
        moves_types = np.ones(4)
        moves_category = np.ones(4)
...
class TrueMaxPlayer(RandomPlayer):
    def choose_move(self, battle):
        # If the player can attack, it will
        if battle.available_moves:
...
if __name__ == "__main__":
    ...
    third_opponent = TrueMaxPlayer(
        player_configuration=PlayerConfiguration("True max player", None),
        battle_format="gen7randombattle",
        server_configuration=LocalhostServerConfiguration,
    )
    ...
    print("\nResults against true max player:")
    env_player.play_against(
        env_algorithm=dqn_evaluation,
